...
Sanghyun Woo<sup>1\*</sup> Shoubhik Debnath<sup>2</sup> Ronghang Hu<sup>2</sup>  
 Xinlei Chen<sup>2</sup> Zhuang Liu<sup>2</sup> In So Kweon<sup>1</sup> Saining Xie<sup>3†</sup>  
<sup>1</sup>KAIST <sup>2</sup>Meta AI, FAIR <sup>3</sup>New York University
...
The diagram illustrates the FCMAE framework. It starts with an 'input' image on the left, which is partially masked. An arrow points to a 'hierarchical encoder' block, which is a series of downsampled feature maps. A label 'sparse conv' points to the encoder. An arrow from the encoder leads to a 'plain decoder' block, which is a single layer. Finally, an arrow points to the 'target' image on the right, which is the reconstructed image. The target image shows the reconstructed parts in a different color (red) compared to the original input.
...
We pre-train and fine-tune using the ImageNet-1K (IN-1K) dataset for 800 and 100 epochs, respectively, and report the top-1 IN-1K validation accuracy for a single  $224 \times 224$  center crop. Additional details about the experimental setup can be found in the appendix.
...
<tr>
<td>Transformer [31]</td>
<td>83.4</td>
<td>8.5</td>
<td>1.5<math>\times</math></td>
</tr>
<tr>
<td>ConvNeXt block</td>
<td><b>83.7</b></td>
<td><b>7.7</b></td>
<td><b>1.7<math>\times</math></b></td>
</tr>
</tbody>
</table>
...
To ease optimization, we add two additional learnable parameters,  $\gamma$  and  $\beta$ , and initialize them to zero. We also add a residual connection between the input and output of the GRN layer. The resulting final GRN block is  $X_i = \gamma * X_i * \mathcal{N}(\mathcal{G}(X)_i) + \beta + X_i$ . This setup allows a GRN layer

<sup>1</sup>To account for the increased number of channels at deeper layers, in practice, we also scale the normalized value by the channel count  $C$ .<table border="1">
<thead>
<tr>
<th>case</th>
<th>ft</th>
...
<tbody>
<tr>
<td><math>(||X_i|| - \mu)/\sigma</math></td>
<td>84.5</td>
</tr>
<tr>
<td><math>1/\sum ||X_i||</math></td>
<td>83.8</td>
</tr>
<tr>
<td><math>||X_i||/\sum ||X_i||</math></td>
<td><b>84.6</b></td>
</tr>
</tbody>
</table>
...
<sup>2</sup>The additional affine parameters  $\gamma/\beta$  are negligible.<table border="1">
<thead>
<tr>
<th>Backbone</th>
<th>Method</th>
...
**ImageNet-22K intermediate fine-tuning.** We also present ImageNet-22K intermediate fine-tuning results [3]. The training process involves three steps: 1) FCMAE pre-<table border="1">
<thead>
<tr>
<th>Type</th>
<th>Backbone</th>
...
**Acknowledgments.** We thank Ross Wightman for the initial design of the small-compute ConvNeXt model variants and the associated training recipe. We also appreciate the helpful discussions and feedback provided by Kaiming He.## Appendix
...
**Pre-training** All models share the same pre-training setup, as noted in Table 8. We use the linear  $lr$  scaling rule [26]:  $lr = base\_lr \times batchsize / 256$ .
...
<td><math>\beta_1, \beta_2=0.9, 0.95</math> [11]</td>
</tr>
<tr>
<td>batch size</td>
<td>4096</td>
...
<td>cosine decay [53]</td>
</tr>
<tr>
<td>warmup epochs [26]</td>
<td>40</td>
</tr>
<tr>
...
<tr>
<td>layer-wise lr decay [3, 16]</td>
<td>0.9</td>
</tr>
<tr>
<td>batch size</td>
...
<tr>
<td>label smoothing [62]</td>
<td>0.2</td>
</tr>
<tr>
<td>mixup [80]</td>
<td>0.0 (A), 0.3 (F/P), 0.5 (N)</td>
</tr>
<tr>
<td>cutmix [79]</td>
<td>0.0 (A), 0.3 (F/P), 0.5 (N)</td>
...
<tr>
<td>drop path [40]</td>
<td>0.1(N/T), 0.2 (B), 0.3 (L), 0.5(H)</td>
</tr>
<tr>
<td>head init [52]</td>
<td>0.001</td>
...
an input resolution of  $512 \times 512$ . In inference, a multi-scale test using resolutions that are  $[0.75, 0.875, 1.0, 1.125, 1.25]$  of  $512 \times 2048$  is employed.
...
<th colspan="2">GRN functions</th>
<th rowspan="2">Val acc.</th>
</tr>
<tr>
<th>aggregation</th>
<th>normalization</th>
...
<tr>
<td>(b) <math>X * \mathcal{G}(X)</math></td>
<td>✓</td>
<td>-</td>
<td>83.9</td>
</tr>
<tr>
<td>(c) <math>X * \mathcal{N}(X)</math></td>
<td>-</td>
<td>✓</td>
<td>unstable</td>
</tr>
<tr>
<td>(d) <math>X * \mathcal{N}(\mathcal{G}(X))</math></td>
<td>✓</td>
<td>✓</td>
<td>84.6</td>
</tr>
...
**GRN component analysis.** The proposed Global Relation Network (GRN) consists of three steps: global feature aggregation, feature normalization, and feature calibration. The main paper demonstrates that the combination of L2-norm based aggregation and divisive normalization works well in practice. Table 16 verifies the individual contribution of these components using ConvNeXt V2-Base as the encoder. When either component is dropped, performance significantly decreases, and the training becomes unstable ifFigure 8. **Masking ratio.** We observe that a masking ratio of 0.6 provides the best result. The y-axis is ImageNet-1K accuracy (%).
...
[7] Thomas Capelle. Finding the New Resnet18. <https://wandb.ai/fastai/fine-tune-timm/reports/Finding-the-New-Resnet18---VmlldzoyMDI0MjU3>, 2022.
...
[17] MMSegmentation contributors. MMSegmentation: Openmmlab semantic segmentation toolbox and benchmark. <https://github.com/open-mmlab/mmsegmentation>, 2020.[18] Spconv Contributors. Spconv: Spatially sparse convolution library. <https://github.com/traveller59/spconv>, 2022.
...
[24] Golnaz Ghiassi, Yin Cui, Aravind Srinivas, Rui Qian, Tsung-Yi Lin, Ekin D Cubuk, Quoc V Le, and Barret Zoph. Simple copy-paste is a strong data augmentation method for instance segmentation. In *CVPR*, 2021.
...
[38] Ronghang Hu, Shoubhik Deb Nath, Saining Xie, and Xinlei Chen. Exploring long-sequence masked autoencoders. *arXiv preprint arXiv:2210.07224*, 2022.
...
[52] Zhuang Liu, Hanzi Mao, Chao-Yuan Wu, Christoph Feichtenhofer, Trevor Darrell, and Saining Xie. A convnet for the 2020s. In *CVPR*, 2022.- [53] Ilya Loshchilov and Frank Hutter. SGDR: Stochastic gradient descent with warm restarts. In *ICLR*, 2017.
...
- [56] Jongchan Park, Sanghyun Woo, Joon-Young Lee, and In So Kweon. Bам: Bottleneck attention module. In *BMVC*, 2018.
