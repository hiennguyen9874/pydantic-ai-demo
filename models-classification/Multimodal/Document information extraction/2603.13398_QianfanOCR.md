...
![Image 1: Refer to caption](https://arxiv.org/html/2603.13398v1/x1.png)
...
![Image 2: Refer to caption](https://arxiv.org/html/2603.13398v1/qianfan_ocr_other_benchmarks.jpg)
...
Pipeline OCR Systems. Pipeline systems Cui et al. [[2025a](https://arxiv.org/html/2603.13398#bib.bib44 "Paddleocr 3.0 technical report")] decompose document parsing into layout detection, element-wise recognition, and rule-based assembly. Recent systems such as PaddleOCR-VL Cui et al. [[2025b](https://arxiv.org/html/2603.13398#bib.bib113 "PaddleOCR-vl: boosting multilingual document parsing via a 0.9b ultra-compact vision-language model")], MonkeyOCR, and MinerU 2.5 pair lightweight detection models with VLM-based recognizers, achieving strong accuracy with modular efficiency. Their key advantage is explicit layout analysis output (bounding boxes, element types), but they suffer from inter-stage error propagation and irreversible loss of visual context during text extraction. Qianfan-OCR recovers the layout analysis capability through Layout-as-Thought while avoiding the pipeline’s cascading error problem.

End-to-End OCR Models. End-to-end approaches directly map document images to structured outputs. Nougat Blecher et al. [[2023](https://arxiv.org/html/2603.13398#bib.bib42 "Nougat: neural optical understanding for academic documents")] demonstrated feasibility on academic papers; GOT-OCR 2.0 Wei et al. [[2024](https://arxiv.org/html/2603.13398#bib.bib115 "General ocr theory: towards ocr-2.0 via a unified end-to-end model")] broadened format support (Markdown, LaTeX, TikZ) at 580M parameters; DeepSeek-OCR Wei et al. [[2025](https://arxiv.org/html/2603.13398#bib.bib88 "DeepSeek-ocr: contexts optical compression")] introduced context optical compression for efficiency; olmOCR Poznanski et al. [[2025](https://arxiv.org/html/2603.13398#bib.bib114 "OlmOCR: unlocking trillions of tokens in pdfs with vision language models")] scaled SFT-based training on large-scale web documents, while its successor olmOCR 2 further introduced GRPO reinforcement learning with unit-test rewards. More recently, Dolphin v2 proposed analyze-then-parse, and Logics-Parsing and Infinity-Parser explored layout-aware RL for structure prediction. These models primarily focus on recognition accuracy or efficiency but lack explicit layout analysis output – a functionality gap that Qianfan-OCR’s Layout-as-Thought addresses. Qianfan-OCR relies on supervised fine-tuning with high-quality layout annotations, a complementary paradigm that future work could augment with reinforcement learning.

General Vision-Language Models. Large VLMs such as Qwen-VL Bai et al. [[2023](https://arxiv.org/html/2603.13398#bib.bib11 "Qwen-vl: a versatile vision-language model for understanding, localization, text reading, and beyond"), [2025](https://arxiv.org/html/2603.13398#bib.bib48 "Qwen2. 5-vl technical report")], InternVL Chen et al. [[2024b](https://arxiv.org/html/2603.13398#bib.bib5 "Internvl: scaling up vision foundation models and aligning for generic visual-linguistic tasks")], Zhu et al. [[2025](https://arxiv.org/html/2603.13398#bib.bib47 "Internvl3: exploring advanced training and test-time recipes for open-source multimodal models")], and Gemini exhibit OCR capabilities as a byproduct of broad multimodal training, but are not optimized for structured document parsing: they incur higher inference costs, lack fine-grained layout control, and underperform specialized systems on structure-sensitive metrics (e.g., table TEDS, reading order accuracy). Qianfan-OCR targets OCR-specialist-level accuracy at comparable inference cost to these models, while additionally supporting explicit layout analysis and prompt-driven task control.

![Image 3: Refer to caption](https://arxiv.org/html/2603.13398v1/x2.png)
...
Qianfan-OCR adopts the proven multi-stage progressive training methodology from Qianfan-VL [Dong et al., [2025](https://arxiv.org/html/2603.13398#bib.bib112 "Qianfan-vl: domain-enhanced universal vision-language models")], which systematically builds model capabilities from basic cross-modal alignment through advanced reasoning tasks. The key adaptation for OCR scenarios lies in the data mixture composition, where we significantly enhance OCR-specific domains while maintaining the overall training framework. The training pipeline consists of four stages with carefully designed data distributions optimized for document intelligence.
...
Table [1](https://arxiv.org/html/2603.13398#S4.T1 "Table 1 ‣ 4.1 Hyperparameters ‣ 4 Training Recipe ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence") summarizes the key hyperparameters for each training stage.
...
Optimizer AdamW (β 1\beta_{1}=0.9, β 2\beta_{2}=0.95)
Weight decay 0.05
...
Prior to training the 4B Qianfan-OCR model, we conduct low-cost ablation studies on Qianfan-VL-8B – a model from the same architectural family that has undergone large-scale general-purpose continual pretraining – to systematically validate our multi-stage training recipe. Starting from a Stage 1 (adapter-only alignment) checkpoint, we evaluate the contribution of each subsequent training stage to OCR performance. All configurations employ full-parameter training, which prior Qianfan-VL experiments have confirmed to be essential for effective domain transfer. Results are summarized in Table [2](https://arxiv.org/html/2603.13398#S4.T2 "Table 2 ‣ 4.2 Ablation Study: Multi-Stage Training Effectiveness ‣ 4 Training Recipe ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence").
...
Specialized OCR Model Benchmarks: We evaluate OCR-specific capabilities using benchmarks designed for specialized document parsing systems: Omni-Doc-Bench v1.5 [Ouyang et al., [2024](https://arxiv.org/html/2603.13398#bib.bib116 "OmniDocBench: benchmarking diverse pdf document parsing with comprehensive annotations")] for diverse PDF document parsing, OLMOCRBench [AllenAI, [2024](https://arxiv.org/html/2603.13398#bib.bib118 "OlmOCR-bench: a comprehensive ocr evaluation benchmark")] for end-to-end document OCR evaluation, CCOCR for multilingual OCR recognition across diverse scripts, and BigDocs [Rodriguez et al., [2024](https://arxiv.org/html/2603.13398#bib.bib16 "Bigdocs: an open dataset for training multimodal models on document and code tasks")] for large-scale document processing tasks. These benchmarks assess the model’s ability to achieve recognition accuracy competitive with specialized OCR systems.

General OCR Capability Benchmarks: To evaluate general-purpose optical character recognition across diverse scenarios, we use OCRBench [Chen et al., [2024a](https://arxiv.org/html/2603.13398#bib.bib12 "OCRBench: on the hidden mystery of ocr in large multimodal models")] and OCRBench v2, comprehensive benchmarks covering scene text, document text, handwriting recognition, formula recognition, and multilingual text across various real-world conditions. These evaluations ensure the model maintains robust OCR performance beyond specialized document parsing scenarios.

Document Understanding Benchmarks: We assess document comprehension and reasoning capabilities using a diverse set of understanding-oriented benchmarks: TextVQA [Singh et al., [2019](https://arxiv.org/html/2603.13398#bib.bib33 "Towards vqa models that can read")] for scene text question answering, DocVQA [Mathew et al., [2021](https://arxiv.org/html/2603.13398#bib.bib9 "Docvqa: a dataset for vqa on document images")] for document visual question answering, CharXiv document question (CharXiv_DQ) and reasoning question (CharXiv_RQ) tasks [Wang et al., [2024](https://arxiv.org/html/2603.13398#bib.bib32 "CharXiv: charting gaps in realistic chart understanding in multimodal llms")] for academic document understanding, ChartQA [Masry et al., [2022](https://arxiv.org/html/2603.13398#bib.bib10 "Chartqa: a benchmark for question answering about charts with visual and logical reasoning")] and ChartQAPro [Masry et al., [2025](https://arxiv.org/html/2603.13398#bib.bib37 "ChartQAPro: a more diverse and challenging benchmark for chart question answering")] for chart interpretation, and ChartBench for comprehensive chart reasoning evaluation. These benchmarks evaluate the model’s ability to perform high-level semantic understanding and reasoning over visual document content, including complex table analysis and chart interpretation.

Key Information Extraction (KIE): We evaluate KIE capabilities across five public benchmarks: OCRBench KIE [Chen et al., [2024a](https://arxiv.org/html/2603.13398#bib.bib12 "OCRBench: on the hidden mystery of ocr in large multimodal models")] (structured field extraction from receipts, invoices, and forms), OCRBenchv2 KIE in both English and Chinese (cross-lingual extraction across diverse document templates), CCOCR KIE (Chinese document field extraction including ID cards, business licenses, and financial documents), and Nanonets KIE (real-world invoice and receipt parsing, measured by F1 score). All scores are normalized to a 0–100 scale. Since specialized OCR models lack native KIE capabilities, we compare against commercial large models (Gemini-3.1-Pro, Gemini-3-Pro, Seed-2.0) and open-source VLMs (Qwen3-4B-VL, Qwen3-VL-235B-A22B).
...
We conduct comprehensive evaluations across OCR-specific benchmarks, general OCR benchmarks, and document understanding tasks. Our evaluation framework is primarily based on VLMEvalKit [Duan et al., [2024](https://arxiv.org/html/2603.13398#bib.bib54 "Vlmevalkit: an open-source toolkit for evaluating large multi-modality models")] with modifications tailored to different benchmark requirements.

Evaluation Methodology. For OCR-specific benchmarks, we follow official metrics or adopt results from recent publications. Specifically, we reference metrics from the OmniDocBench official leaderboard [Ouyang et al., [2024](https://arxiv.org/html/2603.13398#bib.bib116 "OmniDocBench: benchmarking diverse pdf document parsing with comprehensive annotations")], DeepSeek-OCR v2 [Wei et al., [2026](https://arxiv.org/html/2603.13398#bib.bib129 "DeepSeek-ocr 2: visual causal flow")], and PaddleOCR-VL 1.5 [Cui et al., [2026](https://arxiv.org/html/2603.13398#bib.bib128 "PaddleOCR-vl-1.5: towards a multi-task 0.9b vlm for robust in-the-wild document parsing")] papers. For benchmarks with official evaluation scripts (e.g., OmniDocBench v1.5), we directly reuse their implementations.
...
Bench math scans footers column text
Pipeline
PaddleOCR-VL 80.0 85.7 71.0 84.1 37.8 97.0 79.9 85.7 98.5
PaddleOCR-VL-1.5 79.1 84.6 63.8 81.8 36.9 95.5 80.9 90.5 98.9
MonkeyOCR 75.8 83.8 68.8 74.6 36.1 91.2 76.6 80.1 95.3
End-to-end Model
MinerU2.5 75.2 76.6 54.6 84.9 33.7 96.6 78.2 83.5 93.7
Dotsocr 79.1 82.1 64.2 88.3 40.9 94.1 82.4 81.2 99.5
PointsReader 71.8 81.0 74.0 86.0 36.5 19.6 85.2 92.8 99.1
DeepSeek-OCR 77.2 77.2 73.6 80.2 33.3 96.1 66.4 79.4 99.8
Qwen3-VL-4B 79.2 81.9 76.6 79.3 40.7 88.8 78.8 88.0 99.1
Qwen3-VL-2B 70.7 78.1 72.5 69.3 38.0 51.2 76.1 82.1 98.6
Qianfan-OCR (Ours)79.8 80.1 73.1 81.6 42.0 92.2 80.4 89.1 99.6
...
Table [3](https://arxiv.org/html/2603.13398#S6.T3 "Table 3 ‣ 6.1 OCR-Specific Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence") presents detailed performance on OlmOCR Bench across document categories. Qianfan-OCR achieves the highest overall score (79.8) among end-to-end models, competitive with the top pipeline system PaddleOCR-VL (80.0), with strong scores on Base (99.6), Headers & footers (92.2), and Old scans (42.0, best among end-to-end).

Model Overall↑\uparrow Text Edit{}^{\text{Edit}}↓\downarrow Formula CDM{}^{\text{CDM}}↑\uparrow Table TEDs{}^{\text{TEDs}}↑\uparrow Table TEDss{}^{\text{TEDss}}↑\uparrow R-order Edit{}^{\text{Edit}}↓\downarrow
Pipeline
...
Qianfan-OCR (Ours)93.12 0.041 92.43 91.02 93.85 0.049

Table 4: Performance comparison on OmniDocBench. Models are categorized into Pipeline (two-stage) and End-to-end architectures. ↑\uparrow indicates higher is better, ↓\downarrow indicates lower is better. Best results in each section are in bold. Data source: [https://github.com/opendatalab/OmniDocBench](https://github.com/opendatalab/OmniDocBench)

Table [4](https://arxiv.org/html/2603.13398#S6.T4 "Table 4 ‣ 6.1 OCR-Specific Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence") presents results on OmniDocBench v1.5. Among end-to-end models, Qianfan-OCR achieves the highest overall score of 93.12, surpassing DeepSeek-OCR-v2 (91.09), Gemini-3 Pro (90.33), and all other end-to-end models across every sub-metric. Notably, Qianfan-OCR also outperforms several pipeline systems including MinerU2.5 (90.67) and MonkeyOCR-pro-3B (88.85), narrowing the gap with the top pipeline system PaddleOCR-VL 1.5 (94.50).

Analysis of Layout-as-Thought (Thinking Mode). We additionally evaluate Qianfan-OCR with the Layout-as-Thought mechanism enabled at inference time (denoted Qianfan-OCR-think). Compared to the default mode, the thinking variant achieves an overall score of 92.64 vs. 93.12, with per-metric results: Text Edit{}^{\text{Edit}} 0.052 vs. 0.041, Formula CDM{}^{\text{CDM}} 91.92 vs. 92.43, Table TEDs{}^{\text{TEDs}} 91.21 vs. 91.02, Table TEDss{}^{\text{TEDss}} 94.03 vs. 93.85, and R-order Edit{}^{\text{Edit}} 0.051 vs. 0.049. While the aggregate score is slightly lower, the thinking variant shows improvements on table-related metrics (Table TEDs{}^{\text{TEDs}}: +0.19, Table TEDss{}^{\text{TEDss}}: +0.18). More importantly, per-sample analysis reveals that Layout-as-Thought provides targeted benefits on structurally complex documents but can be counterproductive on simpler ones.

To investigate this, we sort all OmniDocBench v1.5 samples by their layout label entropy in descending order and plot the cumulative score as samples are progressively included (Figure [4](https://arxiv.org/html/2603.13398#S6.F4 "Figure 4 ‣ 6.1 OCR-Specific Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence")). Layout label entropy measures the diversity of element types (text, table, formula, figure, etc.) on each page – higher entropy indicates more heterogeneous layouts.

![Image 4: Refer to caption](https://arxiv.org/html/2603.13398v1/x3.png)
...
Illustrative Example. Figure [5](https://arxiv.org/html/2603.13398#S6.F5 "Figure 5 ‣ 6.1 OCR-Specific Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence") presents a concrete example of Layout-as-Thought on a math exam paper with mixed content (text, formulas, geometric diagrams, and multi-column layout). When the <think> token is activated, the model first generates a structured layout analysis enumerating each document element in reading order:

![Image 5: Refer to caption](https://arxiv.org/html/2603.13398v1/show_cases/jiaocaineedrop_jiaocai_needrop_en_2893_nothink_viz.jpg)

![Image 6: Refer to caption](https://arxiv.org/html/2603.13398v1/show_cases/jiaocaineedrop_jiaocai_needrop_en_2893_think_viz.jpg)
...
Table 5: Performance on general OCR benchmarks. Qianfan-OCR achieves competitive scores while maintaining strong understanding capabilities (Table [6](https://arxiv.org/html/2603.13398#S6.T6 "Table 6 ‣ 6.3 Document Understanding Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence")).
...
Table [6](https://arxiv.org/html/2603.13398#S6.T6 "Table 6 ‣ 6.3 Document Understanding Benchmarks ‣ 6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence") compares end-to-end models with two-stage OCR+LLM systems, where specialized OCR models first extract text and Qwen3-4B then generates answers from the extracted text.

End-to-End Models Two-Stage OCR+LLM Systems
Benchmark Qianfan Qwen3 Qwen3 PaddleOCR MinerU2.5 Dotsocr DeepSeek OCR
-OCR-VL-4B-VL-2B+Qwen3-4B+Qwen3-4B+Qwen3-4B+Qwen3-4B
OCRVQA_TESTCORE 66.8 64.7 59.3 20.4 21.2 26.5 19.3
TextVQA_VAL 80.0 81.8 79.9 21.4 16.1 28.0 40.0
DocVQA 92.8 94.9 92.7 59.8 62.0 67.1 55.9
CharXiv_DQ 94.0 81.8 69.7 0.0 0.0 0.0 0.0
CharXiv_RQ 85.2 48.5 41.3 0.0 0.0 0.0 0.0
ChartQA_TEST 88.1 83.3 78.3 56.8 7.0 14.6 23.4
ChartQAPro 42.9 36.2 24.5 20.9 18.7 18.4 18.1
ChartBench 85.9 74.9 73.2 17.2 5.7 2.0 6.1
...
We evaluate KIE performance on five public benchmarks as described in Section [5](https://arxiv.org/html/2603.13398#S5 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). Since specialized OCR models lack native KIE capabilities, we compare Qianfan-OCR with commercial models (Gemini-3.1-Pro, Gemini-3-Pro, Seed-2.0) and open-source models (Qwen3-4B-VL, Qwen3-VL-235B-A22B).

Model Overall OCRBench OCRBenchv2 OCRBenchv2 CCOCR Nanonets
(Mean)KIE KIE (en)KIE (zh)KIE KIE (F1)
...
Qianfan-OCR (W8A8)512 512 1.024
...
*   J. Ainslie, J. Lee-Thorp, M. de Jong, Y. Zemlyanskiy, F. Lebrón, and S. Sanghai (2023)GQA: training generalized multi-query transformer models from multi-head checkpoints. arXiv preprint arXiv:2305.13245. Cited by: [§3.1](https://arxiv.org/html/2603.13398#S3.SS1.p3.1 "3.1 Architecture Overview ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   AllenAI (2024)OlmOCR-bench: a comprehensive ocr evaluation benchmark. Note: [https://huggingface.co/datasets/allenai/olmOCR-bench](https://huggingface.co/datasets/allenai/olmOCR-bench)Cited by: [§5](https://arxiv.org/html/2603.13398#S5.p2.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   J. Bai, S. Bai, S. Yang, S. Wang, S. Tan, P. Wang, J. Lin, C. Zhou, and J. Zhou (2023)Qwen-vl: a versatile vision-language model for understanding, localization, text reading, and beyond. arXiv preprint arXiv:2308.12966. Cited by: [§2](https://arxiv.org/html/2603.13398#S2.p4.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   S. Bai, K. Chen, X. Liu, J. Wang, W. Ge, S. Song, K. Dang, P. Wang, S. Wang, J. Tang, et al. (2025)Qwen2. 5-vl technical report. arXiv preprint arXiv:2502.13923. Cited by: [§2](https://arxiv.org/html/2603.13398#S2.p4.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§3.1](https://arxiv.org/html/2603.13398#S3.SS1.p3.1 "3.1 Architecture Overview ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   L. Blecher, G. Cucurull, T. Scialom, and R. Stojnic (2023)Nougat: neural optical understanding for academic documents. arXiv preprint arXiv:2308.13418. Cited by: [§2](https://arxiv.org/html/2603.13398#S2.p3.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   Y. Chen, L. Zhang, H. Liu, et al. (2024a)OCRBench: on the hidden mystery of ocr in large multimodal models. arXiv preprint arXiv:2305.07895. Cited by: [§5](https://arxiv.org/html/2603.13398#S5.p3.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§5](https://arxiv.org/html/2603.13398#S5.p5.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   Z. Chen, J. Wu, W. Wang, W. Su, G. Chen, S. Xing, M. Zhong, Q. Zhang, X. Zhu, L. Lu, et al. (2024b)Internvl: scaling up vision foundation models and aligning for generic visual-linguistic tasks. arXiv preprint arXiv:2312.14238. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§2](https://arxiv.org/html/2603.13398#S2.p4.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   C. Cui, T. Sun, S. Liang, T. Gao, Z. Zhang, J. Liu, X. Wang, C. Zhou, H. Liu, M. Lin, Y. Zhang, Y. Zhang, Y. Liu, D. Yu, and Y. Ma (2026)PaddleOCR-vl-1.5: towards a multi-task 0.9b vlm for robust in-the-wild document parsing. External Links: 2601.21957, [Link](https://arxiv.org/abs/2601.21957)Cited by: [§6](https://arxiv.org/html/2603.13398#S6.p2.1 "6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   C. Cui, T. Sun, M. Lin, T. Gao, Y. Zhang, J. Liu, X. Wang, Z. Zhang, C. Zhou, H. Liu, et al. (2025a)Paddleocr 3.0 technical report. arXiv preprint arXiv:2507.05595. Cited by: [§2](https://arxiv.org/html/2603.13398#S2.p2.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   C. Cui, T. Sun, M. Lin, T. Gao, Y. Zhang, J. Liu, X. Wang, Z. Zhang, C. Zhou, H. Liu, et al. (2025b)PaddleOCR-vl: boosting multilingual document parsing via a 0.9b ultra-compact vision-language model. arXiv preprint arXiv:2510.14528. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§2](https://arxiv.org/html/2603.13398#S2.p2.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§3.2](https://arxiv.org/html/2603.13398#S3.SS2.p2.1 "3.2 Large-Scale Data Synthesis Pipelines ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   D. Dong, M. Zheng, D. Xu, B. Zhuang, W. Zhang, C. Luo, H. Wang, Z. Zhao, J. Li, Y. Li, H. Zhong, M. Liu, J. Chen, S. Li, L. Tian, Y. Feng, X. Li, D. Jiang, Y. Chen, Y. Xu, D. Qin, C. Feng, D. Wang, H. Zhang, J. Ha, J. He, Y. Zhai, C. Zheng, J. Mao, J. Chen, R. Yao, Z. Yuan, J. Wu, G. Xie, and D. Shen (2025)Qianfan-vl: domain-enhanced universal vision-language models. External Links: 2509.18189, [Link](https://arxiv.org/abs/2509.18189)Cited by: [§3.1](https://arxiv.org/html/2603.13398#S3.SS1.p1.1 "3.1 Architecture Overview ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§3.1](https://arxiv.org/html/2603.13398#S3.SS1.p2.2 "3.1 Architecture Overview ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§4](https://arxiv.org/html/2603.13398#S4.p1.1 "4 Training Recipe ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   H. Duan, J. Yang, Y. Qiao, X. Fang, L. Chen, Y. Liu, X. Dong, Y. Zang, P. Zhang, J. Wang, et al. (2024)Vlmevalkit: an open-source toolkit for evaluating large multi-modality models. In Proceedings of the 32nd ACM International Conference on Multimedia,  pp.11198–11201. Cited by: [§6](https://arxiv.org/html/2603.13398#S6.p1.1 "6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   H. Liu, C. Li, Q. Wu, and Y. J. Lee (2024)Visual instruction tuning. Advances in neural information processing systems 36. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   A. Masry, M. S. Islam, M. Ahmed, A. Bajaj, F. Kabir, A. Kartha, M. T. R. Laskar, M. Rahman, S. Rahman, M. Shahmohammadi, et al. (2025)ChartQAPro: a more diverse and challenging benchmark for chart question answering. arXiv preprint arXiv:2504.05506. Cited by: [§5](https://arxiv.org/html/2603.13398#S5.p4.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
...
*   L. Ouyang, B. Yuan, Z. Yao, F. Li, J. Wu, L. Gao, Z. Zhang, H. Liu, Y. Liu, B. Zhang, et al. (2024)OmniDocBench: benchmarking diverse pdf document parsing with comprehensive annotations. arXiv preprint arXiv:2412.07626. Cited by: [§5](https://arxiv.org/html/2603.13398#S5.p2.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§6](https://arxiv.org/html/2603.13398#S6.p2.1 "6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   J. Poznanski, K. Chandu, D. Schwenk, K. Lo, D. Groeneveld, R. Kinney, L. Soldaini, I. Magnusson, and W. Ammar (2025)OlmOCR: unlocking trillions of tokens in pdfs with vision language models. arXiv preprint arXiv:2502.18443. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§2](https://arxiv.org/html/2603.13398#S2.p3.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
...
*   Z. Wang, M. Xia, L. He, H. Chen, Y. Liu, R. Zhu, K. Liang, X. Wu, H. Liu, S. Malladi, A. Chevalier, S. Arora, and D. Chen (2024)CharXiv: charting gaps in realistic chart understanding in multimodal llms. arXiv preprint arXiv:2406.18521. Cited by: [§5](https://arxiv.org/html/2603.13398#S5.p4.1 "5 Evaluation Framework ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   H. Wei, C. Kong, J. Liang, Z. Chen, Z. Xie, C. Cui, S. Li, Y. Xu, E. Chen, J. Zhao, et al. (2024)General ocr theory: towards ocr-2.0 via a unified end-to-end model. arXiv preprint arXiv:2409.01704. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§2](https://arxiv.org/html/2603.13398#S2.p3.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   H. Wei, Y. Sun, and Y. Li (2025)DeepSeek-ocr: contexts optical compression. arXiv preprint arXiv:2510.18234. Cited by: [§1](https://arxiv.org/html/2603.13398#S1.p1.1 "1 Introduction ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"), [§2](https://arxiv.org/html/2603.13398#S2.p3.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   H. Wei, Y. Sun, and Y. Li (2026)DeepSeek-ocr 2: visual causal flow. External Links: 2601.20552, [Link](https://arxiv.org/abs/2601.20552)Cited by: [§6](https://arxiv.org/html/2603.13398#S6.p2.1 "6 Experimental Results ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   B. Zhang and R. Sennrich (2019)Root mean square layer normalization. In Advances in Neural Information Processing Systems, Vol. 32. Cited by: [§3.1](https://arxiv.org/html/2603.13398#S3.SS1.p3.1 "3.1 Architecture Overview ‣ 3 Model Architecture and Training Data ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence"). 
*   J. Zhu, W. Wang, Z. Chen, Z. Liu, S. Ye, L. Gu, H. Tian, Y. Duan, W. Su, J. Shao, et al. (2025)Internvl3: exploring advanced training and test-time recipes for open-source multimodal models. arXiv preprint arXiv:2504.10479. Cited by: [§2](https://arxiv.org/html/2603.13398#S2.p4.1 "2 Related Work ‣ Qianfan-OCR: A Unified End-to-End Model for Document Intelligence").
